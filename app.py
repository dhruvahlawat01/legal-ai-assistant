...
from user_profile import (
    get_or_create_profile, update_profile, save_analysis,
    get_analysis_history, get_user_stats, delete_analysis,
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            with tab3:
                ...
                with col_a:
                    avg_delivery = st.text_input("⏱️ Average task/delivery time", placeholder="e.g. 7 business days", key="avg_delivery")
                    team_size = st.text_input("👥 Team size", placeholder="e.g. 3 developers, 1 project manager", key="team_size")
                with col_b:
                    current_workload = st.selectbox("📦 Current workload level",
                        ["Low (plenty of capacity)", "Medium (some projects running)", "High (near full capacity)", "Overloaded"], key="workload")
                    revision_time = st.text_input("🔄 Time to process revision requests", placeholder="e.g. 3-5 days per revision round", key="revision_time")

                additional_notes = st.text_area("📝 Any other operational constraints?",
                    placeholder="e.g. Team goes on leave in August, dependent on third-party APIs...",
                    key="op_notes", height=80)
                            ...
                            with st.spinner("🧠 Running breach prediction analysis..."):
                                try:
                                    st.session_state.breach_predictions = analyzer.predict_breach(
                                        st.session_state.breach_obligations, capacity_summary)
                                except Exception as e:
                ...
                if st.session_state.breach_predictions is not None:
                    ...
                    if not st.session_state.breach_predictions:
                        st.success("✅ No high-risk breaches predicted. Your team appears capable of meeting all obligations.")
                    else:
...
def show_profile_page():
    ...
    with col3:
        if st.button("Logout", use_container_width=True):
            for key in ["logged_in", "username", "vectorstore", "chat_history",
                        "analysis_done", "analysis_result", "risk_data"]:
    ...
    history = get_analysis_history(username, limit=20)

    # ── Stats Row ────────────────────────────────────────
    st.subheader("📊 Your Stats")
    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("Contracts Analyzed", stats["total_analyses"])
    c2.metric("Avg Risk Score", f"{stats['avg_risk_score']}%")
    c3.metric("🔴 High Risk Found", stats["high_risk_contracts"])
    c4.metric("🟢 Low Risk Found", stats["low_risk_contracts"])
    c5.metric("Total Clauses Found", stats["total_clauses_found"])
    ...
    with left:
        ...
        with st.form("profile_form"):
            ...
            email = st.text_input("Email Address", value=profile.get("email") or "")
            company = st.text_input("Company / Organisation", value=profile.get("company") or "")
            role = st.selectbox(
                "Your Role",
                ["", "Freelancer", "Small Business Owner", "Startup Founder",
                 "Legal Professional", "Project Manager", "Other"],
                index=0 if not profile.get("role") else
                      (["", "Freelancer", "Small Business Owner", "Startup Founder",
                        "Legal Professional", "Project Manager", "Other"]
                       .index(profile.get("role")) if profile.get("role") in
                       ["Freelancer", "Small Business Owner", "Startup Founder",
                        "Legal Professional", "Project Manager", "Other"] else 0)
            )
            submitted = st.form_submit_button("💾 Save Profile", use_container_width=True)
            if submitted:
                ok, msg = update_profile(username, full_name, email, company, role)
                if ok:
                    st.success(msg)
                else:
                    st.error(msg)
        ...
        st.markdown(f"""
        <div style="background:#f8f9fa;border-radius:10px;padding:16px;font-size:14px;">
            <div style="margin-bottom:8px;"><b>Username:</b> {username}</div>
            <div style="margin-bottom:8px;"><b>Member since:</b> {
                profile.get("created_at", "")[:10] if profile.get("created_at") else "N/A"
            }</div>
            <div><b>Last login:</b> {
                profile.get("last_login", "")[:10] if profile.get("last_login") else "N/A"
            }</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with right:
        ...
        if not history:
            st.info("No contracts analyzed yet. Upload a contract on the dashboard to get started.")
        else:
            for record in history:
                score = record.get("risk_score", 0)
                label = record.get("risk_label", "")
                filename = record.get("filename", "Unknown file")
                analyzed_at = record.get("analyzed_at", "")[:10]
                high = record.get("high_count", 0)
                medium = record.get("medium_count", 0)
                low = record.get("low_count", 0)
                ...
                col_record, col_del = st.columns([9, 1])
                with col_record:
                    st.markdown(f"""
                    <div style="border-left: 4px solid {border_color}; background: {bg_color};
                        ...
                        <div style="font-size:12px; color:#666; margin-top:4px;">
                            {analyzed_at} &nbsp;·&nbsp;
                            <span style="color:#dc3545;">🔴 {high} HIGH</span> &nbsp;
                            <span style="color:#fd7e14;">🟠 {medium} MED</span> &nbsp;
                            <span style="color:#28a745;">🟢 {low} LOW</span>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                with col_del:
                    st.markdown("<div style='margin-top:4px'>", unsafe_allow_html=True)
                    if st.button("🗑️", key=f"del_{record_id}", help="Delete this record"):
